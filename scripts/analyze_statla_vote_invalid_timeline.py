...
def build_rows(election_key: str) -> List[Dict[str, object]]:
    ...
    for commit, committed_at, subject in git_poll_commits(election_key):
        ...
        invalid_votes_erst = max(voters_total - valid_votes_erst, 0)
        invalid_votes_zweit = max(voters_total - valid_votes_zweit, 0)
        invalid_share_erst = (invalid_votes_erst / voters_total * 100.0) if voters_total else 0.0
        invalid_share_zweit = (invalid_votes_zweit / voters_total * 100.0) if voters_total else 0.0
        rows.append(
            {
                "commit": commit,
                ...
                "invalid_votes_erst": invalid_votes_erst,
                "invalid_votes_zweit": invalid_votes_zweit,
                "invalid_share_erst": round(invalid_share_erst, 4),
                "invalid_share_zweit": round(invalid_share_zweit, 4),
            }
        )
    return rows
...
def write_png_report(path: Path, rows: List[Dict[str, object]]) -> None:
    ...
    colors = {
        "invalid_votes_erst": "#0b7a75",
        "invalid_votes_zweit": "#c75146",
        "invalid_share_erst": "#0b7a75",
        "invalid_share_zweit": "#c75146",
    }

    ax_top.plot(df["time_local_dt"], df["invalid_votes_erst"], color=colors["invalid_votes_erst"], linewidth=2.5, label="Ungültige Erststimmen")
    ax_top.plot(df["time_local_dt"], df["invalid_votes_zweit"], color=colors["invalid_votes_zweit"], linewidth=2.5, label="Ungültige Zweitstimmen")
    ax_top.set_ylabel("Absolute Zahl")
    ax_top.yaxis.set_major_formatter(FuncFormatter(format_int))
    ax_top.set_title("Landtagswahl Baden-Württemberg 2026: Ungültige Stimmen im Verlauf des Wahlabends")

    ax_bottom.plot(df["time_local_dt"], df["invalid_share_erst"], color=colors["invalid_share_erst"], linewidth=2.5, label="Ungültige Erststimmen (%)")
    ax_bottom.plot(df["time_local_dt"], df["invalid_share_zweit"], color=colors["invalid_share_zweit"], linewidth=2.5, label="Ungültige Zweitstimmen (%)")
...
def main() -> None:
    ...
    output_csv = Path(args.output_csv) if args.output_csv else report_dir / "statla_vote_invalid_timeline.csv"
    output_png = Path(args.output_png) if args.output_png else report_dir / "statla_vote_invalid_timeline.png"
