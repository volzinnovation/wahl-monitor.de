#!/usr/bin/env python3
"""Refresh latest StatLA exports while preserving the existing komm.one snapshots."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source",
        choices=("live", "presentation"),
        default="live",
        help="Refresh from the live CSV or from the result presentation fallback.",
    )
    parser.add_argument("--result-status", default=None, help="Optional result status stored in latest/run_metadata.json.")
    parser.add_argument("--result-label", default=None, help="Optional result label stored in latest/run_metadata.json.")
...
def main() -> None:
    ...
    if args.source == "presentation":
        ...
        statla = core.fetch_statla(config, config.request_timeout_seconds, force_dummy=False)
        if not statla.get("snapshots"):
            raise SystemExit(
                f"Could not refresh latest StatLA exports from {config.statla_live_csv_url}: "
                f"{statla.get('error_message') or 'no snapshots returned'}"
            )

    if args.result_status:
        statla["result_status"] = args.result_status
    if args.result_label:
        statla["result_label"] = args.result_label
    ...
    print(f"mode={statla.get('mode')}")
    print(f"url={statla.get('url')}")
    print(f"result_status={statla.get('result_status')}")
    print(f"result_label={statla.get('result_label')}")
