...
def append_capture(rows, selected, folder):
    """Append one saved source acquisition without fetching or inventing a Git ref."""
    manifest = json.loads((folder / "manifest.json").read_text())
    require(manifest["election_key"] == "2026-lsa", "Wrong additional election")
    for item in manifest["fetches"]:
        require(Path(item["filename"]).name == item["filename"], "Invalid source filename")
        require(item["status_code"] == 200 and not item["error_message"], "Failed additional source")
        require(digest(folder / item["filename"]) == item["content_hash"], "Additional source checksum differs")
    source = next(x for x in manifest["fetches"]
                  if x["url"].endswith("Ergebnisse_Land_RKR_WKR_LT_2026.csv"))
    with (folder / source["filename"]).open(newline="", encoding="utf-8-sig") as stream:
        land = [r for r in csv.DictReader(stream, delimiter=";")
                if r["Satzart"] == "LAN" and r["Schlüsselnummer"] == "15" and not r["Wahllokal"].strip()]
    require(len(land) == 1, "Expected one additional Land TOTAL row")
    land = land[0]
    total = int(land["F.Gültige.Zweitstimmen"])
    votes, names = {}, {}
    for column, value in land.items():
        match = re.fullmatch(r"F(\d+)\.(.+)", column)
        if match:
            code = "F" + str(int(match[1]))
            votes[code], names[code] = int(value), match[2]
    require(total > 0 and all(v >= 0 for v in votes.values()) and sum(votes.values()) == total,
            "Additional all-party votes differ from denominator")
    final_selected = sorted([
        {"code": code, "party": names[code], "votes": count, "share_percent": 100 * count / total}
        for code, count in votes.items() if count / total > .05
    ], key=lambda x: x["party"].casefold())
    require([x["code"] for x in final_selected] == [x["code"] for x in selected],
            "Additional capture changed the six-party cohort; review the chart selection")
    captured = max(datetime.fromisoformat(x["fetched_at_utc"]) for x in manifest["fetches"])
    local = captured.astimezone(BERLIN)
    row = {
        "capture_index": len(rows) + 1, "commit": None,
        "acquired_at_utc": captured.isoformat(), "acquired_at_local": local.isoformat(),
        "source_time_local": (land.get("Datum", "") + " " + land.get("Uhrzeit", "")).strip(),
        "source_fetched_at_utc": source["fetched_at_utc"], "source_sha256": source["content_hash"],
        "source_git_url": None, "source_local_file": f"additional_sources/{folder.name}/{source['filename']}",
        "valid_votes_zweit": total,
        "reported_precincts": int(land["Ist.Wahlbezirke"]) if land.get("Ist.Wahlbezirke", "").strip() else None,
        "total_precincts": int(land["Soll.Wahlbezirke"]) if land.get("Soll.Wahlbezirke", "").strip() else None,
        "all_party_votes": votes,
    }
    for party in final_selected:
        row[f"{party['party']}_votes"] = party["votes"]
        row[f"{party['party']}_percent"] = party["share_percent"]
        require(0 <= party["share_percent"] <= 60, "Additional bar exceeds fixed axis")
    rows.append(row)
...
def main():
    ...
    parser.add_argument("--extra-capture", type=Path, action="append", default=[],
                        help="Append an already-saved one-off source folder; never fetches")
    ...
    git_capture_count = len(rows)
    extra_captures = [path.resolve() for path in args.extra_capture]
    for folder in extra_captures:
        selected = append_capture(rows, selected, folder)
    suffix = (datetime.fromisoformat(rows[-1]["acquired_at_utc"]).strftime("%Y%m%dT%H%M%SZ")
              if extra_captures else summary["ref"][:8])
    output = (args.output or source.parent / f"animation-{suffix}").resolve()
    require(output != source, "Use a separate animation folder to preserve the frozen audit")
    output.mkdir(parents=True, exist_ok=True)
    for folder in extra_captures:
        destination = output / "additional_sources" / folder.name
    ...
    render_frames(output, selected, rows)
    ...
    manifest = {
        ...
        "git_captures": git_capture_count, "local_captures": len(extra_captures),
        "removed_source_and_status_line": True,
        "additional_captures": [{"directory": f"additional_sources/{folder.name}",
    ...
    last = rows[-1]
    aggregation_note = ""
    if not extra_captures and summary["ref"] == "23377a96d918a4b77f71dbabbf9c385f78334126":
  ...
  [full-history audit]({report_link}/AGGREGATION_ASCHERSLEBEN.md)."""
    if last['reported_precincts'] is None or last['total_precincts'] is None:
        count_note = "The last Land CSV omits its reporting-count fields; these remain missing in the data."
    else:
        count_note = f"The last Land CSV reports **{last['reported_precincts']:,}/{last['total_precincts']:,}** districts."
    extra_command = "".join(" --extra-capture " + shlex.quote(os.path.relpath(path, ROOT)) for path in extra_captures)
    extra_note = (f"{len(extra_captures)} additional local acquisition(s) follow the Git history. "
                  "Their exact downloaded files and manifests are retained in `additional_sources/`; "
                  "they have no fabricated Git commit or GitHub URL."
                  if extra_captures else "No local acquisitions are appended.")
    start_label = datetime.fromisoformat(rows[0]["acquired_at_local"]).strftime("%d.%m.%Y, %H:%M:%S")
    end_label = datetime.fromisoformat(last["acquired_at_local"]).strftime("%d.%m.%Y, %H:%M:%S")
    readme = f"""# Auszählung Landtagswahl Sachsen-Anhalt — Animation

[MPEG-2-Film (.mpg)]({BASENAME}.mpg) · [MP4-Vorschau]({BASENAME}.mp4)

{len(rows)} gespeicherte Abrufe, vom **{start_label} MESZ** bis
**{end_label} MESZ**. 1920 × 1080 Pixel, {args.fps} Bilder/s,
...
- The last Land CSV contains **{last['valid_votes_zweit']:,}** valid second votes.
  {count_note} {aggregation_note} This movie follows
...
- Immutable Git endpoint: `{summary['ref']}` ({git_capture_count} positive-vote captures).
  {extra_note} Each capture in
...
```sh
python3 scripts/render_lsa_results_animation.py --input {shlex.quote(relative_input)}{extra_command} --timeline-seconds {args.timeline_seconds:g} --fps {args.fps}
```
...
codec checks. `frames.ffconcat` and the {len(rows)} PNGs retain the exact rendered inputs.
